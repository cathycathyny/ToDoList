...
public class Workspace extends AppWorkspaceComponent {
	...
	// FIRST THE LABEL AT THE TOP
        ...
        //itemsTable.getColumns().addAll(itemCategoryColumn, itemDescriptionColumn, itemStartDateColumn, itemEndDateColumn, itemCompletedColumn);
...
// AND NOW SETUP THE WORKSPACE
	...
	DataManager dataManager = (DataManager)app.getDataComponent();
        //update change of new todo list name
        nameTextField.textProperty().addListener((observable, oldValue, newValue) -> {
             dataManager.setName(newValue);
        });
        
        //update change of new owner name
        ownerTextField.textProperty().addListener((observable, oldValue, newValue) -> {
             dataManager.setOwner(newValue);
        });
	...
	// NOW CONNECT THE BUTTONS TO THEIR HANDLERS
        ...
        removeItemButton.setOnAction(e->{
            //verify if the user want to remove item
            AppYesNoCancelDialogSingleton yesNoDialog = AppYesNoCancelDialogSingleton.getSingleton();
            yesNoDialog.show("Remove Item", "Are you sure to remove this item?");
        
            String selection = yesNoDialog.getSelection();
        
            if (selection.equals(AppYesNoCancelDialogSingleton.YES)) {
                ToDoItem removeItem = (ToDoItem) itemsTable.getSelectionModel().getSelectedItem();
                toDoListController.processRemoveItem(removeItem);
            }
            gui.updateButtonDisable(removeItemButton);
            gui.updateButtonDisable(moveUpItemButton);
            gui.updateButtonDisable(moveDownItemButton);
        });
        
        
        moveUpItemButton.setOnAction(e->{
            ToDoItem editItem = (ToDoItem) itemsTable.getSelectionModel().getSelectedItem();
            toDoListController.processMoveUpItem(editItem);
            itemsTable.getSelectionModel().select(editItem);
            gui.updateButtonVisble(moveUpItemButton);
            gui.updateButtonVisble(moveDownItemButton);
            if(dataManager.getIndex(editItem)==0){
                gui.updateButtonDisable(moveUpItemButton);
                gui.updateButtonVisble(moveDownItemButton);
            }else if (dataManager.getIndex(editItem)==(dataManager.getSize()-1)){
                gui.updateButtonVisble(moveUpItemButton);
                gui.updateButtonDisable(moveDownItemButton);
            }
        });
        ...
        moveDownItemButton.setOnAction(e->{
            ToDoItem editItem = (ToDoItem) itemsTable.getSelectionModel().getSelectedItem();
            toDoListController.processMoveDownItem(editItem);
            itemsTable.getSelectionModel().select(editItem);
            gui.updateButtonVisble(moveUpItemButton);
            gui.updateButtonVisble(moveDownItemButton);
            if(dataManager.getIndex(editItem)==0){
        ...
        itemsTable.setRowFactory(new Callback<TableView<ToDoItem>, TableRow<ToDoItem>>() {  
        @Override  
            public TableRow<ToDoItem> call(TableView<ToDoItem> tableView2) {  
                final TableRow<ToDoItem> row = new TableRow<>();
                row.addEventFilter(MouseEvent.MOUSE_PRESSED, new EventHandler<MouseEvent>() {  
                    @Override  
                    public void handle(MouseEvent event) {
                        // Enable and disable the remove button
                        if(dataManager.getItems().isEmpty()){
                            gui.updateButtonDisable(removeItemButton);
                        }else{
                            gui.updateButtonVisble(removeItemButton);
                        }
                    
                    final int index = row.getIndex();
                    // Enable and disable move up and move down button
                    gui.updateButtonDisable(moveUpItemButton);
                    gui.updateButtonDisable(moveDownItemButton);
                    if(dataManager.getSize()>1){
                        gui.updateButtonVisble(moveUpItemButton);
                        gui.updateButtonVisble(moveDownItemButton);
                        if(index==0){
                            gui.updateButtonDisable(moveUpItemButton);
                            gui.updateButtonVisble(moveDownItemButton);
                        }else if (index==itemsTable.getItems().size()-1){
                            gui.updateButtonVisble(moveUpItemButton);
                            gui.updateButtonDisable(moveDownItemButton);
                        }
                    }
                    
                    //handle edit item and edge case
                    if (index >= 0 && index < itemsTable.getItems().size() && itemsTable.getSelectionModel().isSelected(index)  ) {
                        if (event.getClickCount() == 2) {
                            ToDoItem editItem = (ToDoItem) itemsTable.getSelectionModel().getSelectedItem();
                            toDoListController.processEditItem(editItem);
                        }
